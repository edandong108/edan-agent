...
- [ ] 9. 提交后若要继续本机联调：跑 `scripts\start-backend-testdb.ps1`（内含 `ensure-local-backend`）；或一次性安装 `核心文档\本地联调\install-dev-hooks.ps1` 自动恢复。详见 `.cursor/rules/local-dev-bootstrap.mdc`
```
